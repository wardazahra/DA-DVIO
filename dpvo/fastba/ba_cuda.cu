                  ...
                  int _t0, int _t1, int _iterations, bool eff_impl)
{
  ...
  // auto opts = poses.options();//这个应该是tensor的数据类型
  auto opts = torch::TensorOptions()
    .dtype(torch::kFloat32).device(torch::kCUDA);
...
{
  // std::cout<<" Eenter hessian"<<std::endl;
    const int N = t1 - t0;    // number of poses
    const int M = kx.size(0); // number of patches
    const int num = ii.size(0);//所有边的数量
  ...
  // std::cout<<" reprojection_residuals_and_hessian"<<std::endl;
    ...
    if (t1 - t0 == 0) {
      ...
      if (eff_impl) {
        ...
        // S_cpu=B.to(torch::kCPU).to(torch::kFloat64);//转到cpu上
        S_cpu=S.to(torch::kCPU).to(torch::kFloat64);//! 原版可能出错的地方
        ...
        torch::Tensor S = B - torch::matmul(EQ, Et);//cholesky_solve求解的A矩阵：对应Hgg
        torch::Tensor y = v - torch::matmul(EQ,  u);//cholesky_solve求解的b矩阵：对应vgg

        S += I * (1e-4 * S + 1.0);//这个才是A矩阵
      ...
      // std::cout<<" finish"<<std::endl;
      // 直接将 S 和 y 的结果复制到 Hgg 和 vgg 中
      Hgg.copy_(S_cpu);//由于 Hgg 和 vgg 是从 Python 传入的，因此这种直接修改会在 Python 端生效。
      // vgg.copy_(y_cpu);
      vgg.copy_(y_cpu.squeeze(1));// 移除第二个维度，y 的形状将变为 [60]
    }
...
std::vector<torch::Tensor> BAFactor::retract(torch::Tensor _dx)
{
    ...
    // std::cout<<"输入的_dx size"<<_dx.sizes()<<std::endl;
    // std::cout<<"输入的_dx"<<_dx<<std::endl<<std::endl<<std::endl;
    
    // torch::Tensor dX = _dx;//传入的dx可以理解为计算的位姿增益。这个是从Python传入的
    ...
    // std::cout<<"获得的dX size"<<dX.sizes()<<std::endl;
    

    torch::Tensor Q = 1.0 / (C + lmbda).view({1, M});//C为init函数获取的。hessian中更新了M
    torch::Tensor Qt = torch::transpose(Q, 0, 1);//M
    ...
    if (eff_impl) {
      ...
      torch::Tensor Et = torch::transpose(E, 0, 1);//E为init函数获取的。hessian中更新了。6*N, 1*M
      dZ = Qt * (u - torch::matmul(Et, dX));//u为init函数获取的。hessian中更新了
    }

    dX = dX.view({N, 6});
    dZ = dZ.view({M});

    pose_retr_kernel<<<NUM_BLOCKS(N), NUM_THREADS>>>(t0, t1,//为init函数获取的
        poses.packed_accessor32<float,2,torch::RestrictPtrTraits>(),//为init函数获取的。hessian中可能更新了
        dX.packed_accessor32<float,2,torch::RestrictPtrTraits>());

    patch_retr_kernel<<<NUM_BLOCKS(M), NUM_THREADS>>>(
        kx.packed_accessor32<long,1,torch::RestrictPtrTraits>(),//kx为init函数获取的
        patches.packed_accessor32<float,4,torch::RestrictPtrTraits>(),//为init函数获取的。hessian中可能更新了
        dZ.packed_accessor32<float,1,torch::RestrictPtrTraits>());//当前获取的，或者当前传入的
