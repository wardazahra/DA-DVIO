...
class VONet(nn.Module):
    ...
    def forward(self, images, poses, disps, intrinsics, M=1024, STEPS=12, P=1, structure_only=False, rescale=False, patch_callback=None):
        ...
        # ============================================
        # CHANGED: Use stereo depth for initialization
        # ============================================
        if disps is not None:
            # Stereo depth available: use it for metric scale initialization
            patches = set_depth(patches, d)
        else:
            # No stereo depth: fallback to random initialization (monocular)
        ...
        while len(traj) < STEPS:
            ...
            if len(traj) >= 8 and n < images.shape[1]:
                ...
                # 🔥 NEW: Apply callback before BA
            if patch_callback is not None:
                patches = patch_callback(patches, ix)
