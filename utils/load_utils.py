...
def get_real_data_list(evs_slicer, tss_imgs_us, intrinsics, rectify_map, trafos, dT_ms, Horig, Worig):
    ...
    for i, ts_us in enumerate(tss_imgs_us): 
        ...
        data_list.append((voxel, intrinsics, t1_us))  #时间戳改为结束时间戳
    return data_list
...
def load_gt_us(path, skiprows=0):
    traj_ref = np.loadtxt(path, delimiter=" ", skiprows=skiprows)
    tss_gt_us = traj_ref[:, 0].copy() #拿到时间戳
    assert np.all(tss_gt_us == sorted(tss_gt_us))
    assert traj_ref.shape[0] > 0
    assert traj_ref.shape[1] == 8

    return tss_gt_us, traj_ref[:, 1:]#返回时间戳和位姿，位姿为xyz xyzw
...
def mvsec_evs_iterator(scenedir, side="left", stride=1, dT_ms=None, timing=False, H=260, W=346):
    ...
    folder_name = scenedir.split('/')[-1]
    # tss_imgs_us = sorted(np.loadtxt(os.path.join(scenedir, folder_name, f"tss_imgs_us_{side}.txt")))
    assert num_imgs == len(tss_imgs_us)

    # rect_file = osp.join(scenedir, f"rectify_map_{side}.h5")
    rect_file = osp.join(scenedir, folder_name, f"rectify_map_{side}.h5")#鱼眼镜头
    rectify_map = read_rmap(rect_file, H=H, W=W)
...
def vector_evs_h5_iterator(indir, side="left", depth=False, **kwargs):
    intrinsics = np.loadtxt(os.path.join(indir, f"calib_undist_evs_{side}.txt"))
    assert len(intrinsics) == 4
    intrinsics = torch.from_numpy(intrinsics).cuda()

    voxel_h5 = os.path.join(indir, f"voxels_undistorted_{side}.h5")
    if not os.path.exists(voxel_h5):
        raise FileNotFoundError(f"Voxel file {voxel_h5} not found. Please generate the cache voxel file first or use the non-h5 iterator.")
    datain = h5py.File(voxel_h5, 'r')

    num = datain['t_us_range'].shape[0]
    if depth:
        depth_files = sorted(glob.glob(os.path.join(indir, "evs_depth", "*.png")))
        depth_us_list = [get_timestamp_from_filename(file) for file in depth_files]

    for i in range(num):
        voxel = torch.from_numpy(datain['voxels'][i])
        t0_us, t1_us = datain['t_us_range'][i]
        ts_us = t1_us  # use t1_us as the timestamp to make sure the timestamp is aligned with imu data
        if depth:
            depth_idx = find_matching_depth_idx(ts_us, depth_us_list)
            if depth_idx is None:
                depth_img = None
            else:
                depth_file = depth_files[depth_idx]
                depth_img = cv2.imread(depth_file, cv2.IMREAD_UNCHANGED)
                depth_img = depth_img.astype(np.float32) / 1000.0  # mm to m
                depth_img = torch.from_numpy(depth_img).cuda()
            yield (voxel.cuda(), depth_img), intrinsics, ts_us
        else:
            yield voxel.cuda(), intrinsics, ts_us
...
def get_timestamp_from_filename(file):
    """
    Args:
        file: filename or file path including the file name, e.g. "1642644847.028356.png"
    """
    timestamp_str = os.path.splitext(os.path.basename(file))[0]  # unit: s
    timestamp_us = float(timestamp_str) * 1e6
    return timestamp_us


def find_matching_depth_idx(ts_us, depth_us_list, max_diff_s=1):
    """
    Find the depth image with the closest timestamp to the event timestamp.
    """
    depth_idx = np.argmin(np.abs(np.array(depth_us_list) - ts_us))
    if np.abs(depth_us_list[depth_idx] - ts_us) > max_diff_s * 1e6:
        return None
    return depth_idx

def scene_in_list(scene, scene_list):
    return any(x in scene for x in scene_list)

def load_vector_imu_data(indir, side='left'):
    # Load IMU-Camera Extrinsics (from Camera to IMU)
    scene = os.path.basename(indir)
    small_scenes = ["corner_slow", "desk_fast", "desk_normal", "hdr_fast", "hdr_normal", "mountain_fast", "mountain_normal", "robot_fast", "robot_normal", "sofa_fast", "sofa_normal"]
    is_small_scene = scene_in_list(scene, small_scenes)
    ...
    if is_small_scene:
            ...
            T_cam0_cam = np.array(
                [ 0.9999407352369797  , 0.009183655542749752,  0.005846920950435052,  0.0005085820608404798, 
                 ...
                 -0.005928253827254812, 0.008854151768176144,  0.9999432282899994  , -0.0140781304960408   ,
                  0                   , 0                   ,  0                   ,  1                    ]
            ).reshape([4, 4])  # from cam2 to cam0
        elif side == 'right':
            T_cam0_cam = np.array(
                [ 0.9998950454303925  , -0.01084408450654509 , -0.00960749472903985 ,  0.1707305331537136 , 
                  0.01077066454096101 ,  0.9999126470245011  , -0.007661012061206358, -0.04041847718387014,
                  0.009689732147985561,  0.007556728900177175,  0.9999244996194612  , -0.01730941104291421,
                  0                   ,  0                   ,  0                   ,  1                  ]
            ).reshape([4, 4])  # from cam3 to cam0
    else:
        # lidar seq
        T_cam0_imu = np.array(
        ...
        if side == 'left':
            ...
            T_cam0_cam = np.array(
                [ 0.9999556435596104  , -0.008932688408805093,  0.0029863005670183,  0.1709356441270855 ,
    ...
    T_imu_cam = np.linalg.inv(T_cam0_imu) @ T_cam0_cam

    # return T_imu_cam

    # 在返回前添加展平操作
    T_imu_cam_flat = T_imu_cam.flatten().tolist()
    return T_imu_cam_flat
...
#读取240c的数据
def davis240c_evs_iterator(indir, side="left", stride=1, timing=False, dT_ms=None, H=180, W=240):
    if timing:
        t0 = torch.cuda.Event(enable_timing=True)
        t1 = torch.cuda.Event(enable_timing=True)
        t0.record()
    ...
    tss_imgs_us = np.loadtxt(os.path.join(indir, f"tss_imgs_us_{side}.txt"))#图像的时间戳
    ...
    if timing:  
        ...
        torch.cuda.synchronize()
        dt = t0.elapsed_time(t1)/1e3
        print(f"Preloaded {len(data_list)} davis240c voxels in {dt} secs, e.g. {len(data_list)/dt} FPS")
    print(f"Preloaded {len(data_list)} davis240c voxels, imstart={imstart}, imstop={imstop}, stride={stride}, dT_ms={dT_ms} on {indir}")

    for (voxel, intrinsics, ts_us) in data_list:
        yield voxel.cuda(), intrinsics.cuda(), ts_us

def davis240c_evs_h5_iterator(indir, side="left", **kwargs):
    intrinsics = np.loadtxt(os.path.join(indir, f"calib_undist_{side}.txt"))
    assert len(intrinsics) == 4
    intrinsics = torch.from_numpy(intrinsics).cuda()
...
#读取monohku的数据
def monohku_evs_iterator(indir, side="davis346", stride=1, timing=False, dT_ms=None, H=260, W=346):
    if timing:
        t0 = torch.cuda.Event(enable_timing=True)
        t1 = torch.cuda.Event(enable_timing=True)
        t0.record()
    ...
    tss_imgs_us = np.loadtxt(os.path.join(indir, f"tss_imgs_us_davis346.txt"))#图像的时间戳(注意只有davis346有图像)
    ...
    if timing:  
        ...
        torch.cuda.synchronize()
        dt = t0.elapsed_time(t1)/1e3
        print(f"Preloaded {len(data_list)} monohku voxels in {dt} secs, e.g. {len(data_list)/dt} FPS")
    print(f"Preloaded {len(data_list)} monohku voxels, imstart={imstart}, imstop={imstop}, stride={stride}, dT_ms={dT_ms} on {indir}")

    for (voxel, intrinsics, ts_us) in data_list:
        yield voxel.cuda(), intrinsics.cuda(), ts_us

#读取DSEC的数据
def dsec_evs_iterator(indir, side="davis346", stride=1, timing=False, dT_ms=None, H=480, W=640):
    if timing:
        t0 = torch.cuda.Event(enable_timing=True)
        t1 = torch.cuda.Event(enable_timing=True)
        t0.record()
    ...
    tss_imgs_us = np.loadtxt(os.path.join(indir, f"tss_imgs_us_cam00.txt"))#图像的时间戳(注意是图像的时间)
...
# 直接读取voxel的
def fpv_evs_h5_iterator(scenedir, **kwargs):
    intrinsics = load_intrinsics_ecd(scenedir)
    fx, fy, cx, cy = intrinsics 
    intrinsics = torch.from_numpy(np.array([fx, fy, cx, cy])).cuda()
    
    voxel_h5 = os.path.join(scenedir, f"voxels_undistorted.h5")
    if not os.path.exists(voxel_h5):
        raise FileNotFoundError(f"Voxel file {voxel_h5} not found. Please generate the cache voxel file first or use the non-h5 iterator.")
    datain = h5py.File(voxel_h5, 'r')
...
def get_calib_fpv(indir):
    ...
    if "indoor_45_" in indir:
        ...
        T_cam_imu = np.array([[ 0.9998829655327196, 0.005335413966337045, -0.014338360969823338, -0.0015224098391112568],
                              ...
                              [ 0.0, 0.0, 0.0, 1.0]])
    elif "outdoor_45_" in indir:
        K = np.array([171.3189331749973, 171.16692506281044, 163.16606072570642, 134.5434626825864])
        D = np.array([-0.029972792860434635, 0.0006320907049280105, -0.0031606524316315924, 0.0002443885128981053])
        T_cam_imu = np.array([[ 0.9999981428301098, 0.0019147782261195044, 0.00021899926037192538, 0.001766255281872623],
                              [-0.0019146079880968678, 0.9999978668768189, -0.00077493100586091, -0.0026482043960448266],
                              [ -0.00022048261423629378, 0.0007745102689490837, 0.9999996757605775, -0.0277503831648819],
                              [ 0.0, 0.0, 0.0, 1.0]])
    else:
