...
def ate_real(traj_ref, tss_ref_us, traj_est, tstamps):
    ...
    try:
        ...
        if skip_frames > 0:
            ...
            ape_trans = main_ape.ape(evoGT_truncated, evoEst_truncated, pose_relation=metrics.PoseRelation.translation_part, align=True, correct_scale=False)
        else:
            ape_trans = main_ape.ape(evoGT, evoEst, pose_relation=metrics.PoseRelation.translation_part, align=True, correct_scale=False)
    except:
...
def log_results(data, hyperparam, all_results, results_dict_scene, figures, 
    ...
    try:
        ...
        if _n_to_align == -1:
            ...
            if skip_frames > 0:
                ...
                ape_trans = main_ape.ape(copy.deepcopy(evoGT), copy.deepcopy(evoEst), pose_relation=metrics.PoseRelation.translation_part, align=True, correct_scale=False)
        else:
    ...
    if plot:
        ...
        plot_trajectory((traj_est, tss_est_us/1e6), (traj_GT, tss_GT_us/1e6), 
                        ...
                        pdfname, align=True, correct_scale=False, max_diff_sec=max_diff_sec)
...
from evo.core.geometry import GeometryException
import matplotlib.pyplot as plt
def plot_four_trajectory(pred_traj, ESVO_AA_traj, ESVO_traj, gt_traj=None, title="", filename="", align=True, correct_scale=False, max_diff_sec=0.01):
...
def show_trajectory_comparison(pred_traj, ESVO_AA_traj=None, ESVO_traj=None, ESVIO_traj=None, ESIO_traj=None, gt_traj=None, align=True, _n_to_align=-1,correct_scale=False, max_diff_sec=0.01,title="", filename=""):
    ...
    if ESVIO_traj is not None:
        ...
        ape_trans = main_ape.ape(copy.deepcopy(gt_traj_esvio), copy.deepcopy(ESVIO_traj), pose_relation=metrics.PoseRelation.translation_part, align=True,n_to_align=_n_to_align, correct_scale=False)
    ...
    if ESIO_traj is not None:
        ...
        ape_trans = main_ape.ape(copy.deepcopy(gt_traj_esio), copy.deepcopy(ESIO_traj), pose_relation=metrics.PoseRelation.translation_part, align=True,n_to_align=_n_to_align, correct_scale=False)
