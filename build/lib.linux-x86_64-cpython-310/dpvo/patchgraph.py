...
class PatchGraph:
    ...
    def __init__(self, cfg, P, DIM, pmem, **kwargs):
        ...
        self.N = self.cfg.BUFFER_SIZE #缓冲区大小

        self.tstamps_ = np.zeros(self.N, dtype=np.int64)#只是索引数字，记录关键帧的索引在实际对应的位置
        ...
        self.poses_[:,6] = 1.0 #第6个元素是1，表示w=1，那么必然就是xyz xyzw的形式
    ...
    def edges_loop(self):
        ...
        lc_range = self.cfg.MAX_EDGE_AGE #最大的参与回环检测的帧数
        ...
        mask = (flow_mag < self.cfg.BACKEND_THRESH)#对于光流阈值小于64的点
