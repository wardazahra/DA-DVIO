...
def compute_distance_matrix_flow(poses, disps, intrinsics):
    ...
    if not isinstance(poses, SE3):
        ...
        poses = SE3(poses).inv() # [simon] this does c2w => w2c (locally)
    ...
    ii = ii.reshape(-1).cuda() # list of frames i
    jj = jj.reshape(-1).cuda() # list of frames j
    ...
    for i in range(0, ii.shape[0], s):
        flow1, val1 = induced_flow(poses, disps, intrinsics, ii[i:i+s], jj[i:i+s]) # flow ii -> jj
        flow2, val2 = induced_flow(poses, disps, intrinsics, jj[i:i+s], ii[i:i+s]) # flow jj -> ii
