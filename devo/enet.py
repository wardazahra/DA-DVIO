...
class Patchifier(nn.Module):
    def __init__(self, args, patch_size=3, dim_inet=DIM, dim_fnet=128, dim=32, patch_selector=SelectionMethod.SCORER):
        ...
        if args.resnet:#改为采用ResNetFPN
            self.fnet = ResNetFPN(args, input_dim=5, output_dim=self.dim_fnet, norm_layer=nn.BatchNorm2d, init_weight=True)# matching-feature extractor
            self.inet = ResNetFPN(args, input_dim=5, output_dim=self.dim_inet, norm_layer=nn.BatchNorm2d, init_weight=True)  # context-feature extractor
        else: #原版本DEVO中的特征提取网络
...
class eVONet(nn.Module):
    def __init__(self, args, P=3, use_viewer=False, dim_inet=DIM, dim_fnet=128, dim=32, patch_selector=SelectionMethod.SCORER, norm="std2", randaug=False):
        ...
        # 对patch进行处理的
        self.patchify = Patchifier(args, patch_size=self.P, dim_inet=self.dim_inet, dim_fnet=self.dim_fnet, dim=dim, patch_selector=patch_selector)
        # 进行更新操作
