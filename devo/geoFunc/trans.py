...
def FromTwoVectors(a,b):
    ...
    if c > 0.9999:  # vectors are nearly the same
        return np.eye(3)
    elif c < -0.9999:  # vectors are nearly opposite
        # Find orthogonal vector
        if abs(v0[0]) < 0.9:
            orthogonal = np.cross(v0, np.array([1, 0, 0]))
        else:
            orthogonal = np.cross(v0, np.array([0, 1, 0]))
        orthogonal = orthogonal / np.linalg.norm(orthogonal)
        return Rotation.from_rotvec(np.pi * orthogonal).as_matrix()
